...
import it.csi.nivola.nivolasp.integration.remedy.invoker.auth.ApiKeyAuth;
import it.csi.nivola.nivolasp.integration.remedy.invoker.auth.Authentication;
import it.csi.nivola.nivolasp.integration.remedy.invoker.auth.HttpBasicAuth;
import it.csi.nivola.nivolasp.integration.remedy.invoker.auth.OAuth;


@Component("it.csi.nivola.nivolasp.integration.remedy.invoker.ApiClient")
public class ApiClient {
	...
	@Autowired
	public RemedyApiConfig remedyApiConfig;
    ...
    public void impostazione() {
    	setBasePath(remedyApiConfig.getApiUrl());
    }
    
    public String getToken(boolean refresh) {
    	if (token == null || refresh)
    		token = generazioneToken.richiediToken();
    	return token;
    }
    ...
    */
    ...
    public <T> T invokeAPI(String path, HttpMethod method, MultiValueMap<String, String> queryParams, Object body, HttpHeaders headerParams, MultiValueMap<String, Object> formParams, List<MediaType> accept, MediaType contentType, String[] authNames, ParameterizedTypeReference<T> returnType) throws RestClientException {
        ...
        boolean tokenToBeRefreshed = false;
        do {
	        ...
	        headerParams.add(HttpHeaders.AUTHORIZATION, "Bearer " + getToken(tokenToBeRefreshed));// 49702274-90f8-3eb8-b4f6-30da9953560d
	        ...
	        try {
            	...
            	if (e.getStatusCode().equals(HttpStatus.UNAUTHORIZED) && e.getResponseBodyAsString().indexOf("expired") != -1) {
            		tokenToBeRefreshed = true;
            	} else {
            		throw e;
            	}
            }
        } while (tokenToBeRefreshed);
        return null;
    }
    ...
    private class ApiClientHttpRequestInterceptor implements ClientHttpRequestInterceptor {
        ...
        public ClientHttpResponse intercept(HttpRequest request, byte[] body, ClientHttpRequestExecution execution) throws IOException {
            String requestId = logRequest(request, body);
            ClientHttpResponse response = execution.execute(request, body);
            logResponse(response, requestId);
            return response;
        }

        private String logRequest(HttpRequest request, byte[] body) throws UnsupportedEncodingException {
        	String requestId = UUID.randomUUID().toString();
        	StringBuilder logPulito = new StringBuilder("\n---------------- REMEDY REQUEST BEGIN "+ requestId +" ----------------");
			logPulito.append("\n\tURI: " + request.getURI());
			...
			if (body.length < 10240)
				...
				logPulito.append("\n\tRequest Body: <multipart/allegato di grosse dimensioni>");
			logPulito.append("\n---------------- REMEDY REQUEST END ----------------\n");
			log.debug(logPulito.toString());
			return requestId;
        }

        private void logResponse(ClientHttpResponse response, String requestId) throws IOException {
        	StringBuilder logPulito = new StringBuilder("\n---------------- REMEDY RESPONSE BEGIN "+ requestId + " ----------------");
        	...
        	logPulito.append("---------------- REMEDY RESPONSE END ----------------\n");
