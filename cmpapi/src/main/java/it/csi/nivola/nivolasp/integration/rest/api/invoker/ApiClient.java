...
import it.csi.nivola.nivolasp.restinvoker.JwtTokenHelperInterface;

@javax.annotation.Generated(value = "io.swagger.codegen.languages.JavaClientCodegen", date = "2018-04-19T12:20:38.618+02:00")
@Component("it.csi.nivola.nivolasp.integration.invoker.ApiClient")
public class ApiClient {
	private final Logger log = LoggerFactory.getLogger(ApiClientHttpRequestInterceptor.class);
    ...
    @Autowired
    JwtTokenHelperInterface jwtTokenHelper;
    
    boolean primaVoltaToken = true;
    ...
    */
     ...
     */
    public <T> ResponseEntity<T> invokeAPI(String path, HttpMethod method, MultiValueMap<String, String> queryParams, Object body, HttpHeaders headerParams, MultiValueMap<String, Object> formParams, List<MediaType> accept, MediaType contentType, String[] authNames, ParameterizedTypeReference<T> returnType, boolean isTokenNeeded) throws RestClientException {
        updateParamsForAuth(authNames, queryParams, headerParams);
        ResponseEntity<T> responseEntity = null;
        
        final UriComponentsBuilder builder = UriComponentsBuilder.fromHttpUrl(jwtTokenHelper.getBasePath()).path(path);
        if (queryParams != null) {
            builder.queryParams(queryParams);
        }
        
    	//alla prima chiamata dallo startup dell'applicazione il token e' sicuramente da generare
    	boolean refreshToken = false || primaVoltaToken;
    	primaVoltaToken = false;
    	do {
            ...
            //sezione di gestione del token
            AccessTokenResponse accessToken = jwtTokenHelper.retreiveAccessTokenJwt(path, refreshToken);
            refreshToken = false;
            headerParams.remove("Authorization");
            headerParams.set("Authorization", accessToken.getTokenType() + " " + accessToken.getAccessToken());
            ...
            try {
            	...
            	if (e.getStatusCode().equals(HttpStatus.UNAUTHORIZED) && e.getResponseBodyAsString().indexOf("expired") != -1) {
            		refreshToken = true;
            		accessToken = null;
            	} else {
            		throw e;
    ...
    public <T> ResponseEntity<T> invokeAPI(String path, HttpMethod method, MultiValueMap<String, String> queryParams, Object body, HttpHeaders headerParams, MultiValueMap<String, Object> formParams, List<MediaType> accept, MediaType contentType, String[] authNames, ParameterizedTypeReference<T> returnType) throws RestClientException {
    	return invokeAPI(path, method, queryParams, body, headerParams, formParams, accept, contentType, authNames, returnType, true);
    }
    ...
    protected RestTemplate buildRestTemplate() {
        ...
        for (HttpMessageConverter<?> elem : restTemplate.getMessageConverters()) {
        	if (elem instanceof MappingJackson2HttpMessageConverter) {
        		MappingJackson2HttpMessageConverter json = (MappingJackson2HttpMessageConverter) elem;
        		ObjectMapper om = json.getObjectMapper().setSerializationInclusion(Include.NON_NULL);
    ...
    private class ApiClientHttpRequestInterceptor implements ClientHttpRequestInterceptor {
        ...
        public ClientHttpResponse intercept(HttpRequest request, byte[] body, ClientHttpRequestExecution execution) throws IOException {
            ...
            //jwtTokenHelper.logInvocazioneCmp(request, body, response);
            return response;
        }

		private void logRequest(HttpRequest request, byte[] body, String requestId) throws UnsupportedEncodingException {
			StringBuilder logPulito = new StringBuilder("\n========== REQUEST BEGIN "+ requestId +"  ==========");
			logPulito.append("\n\tURI: " + request.getURI());
			logPulito.append("\n\tHTTP Method: " + request.getMethod());
			logPulito.append("\n\tHTTP Headers: " + headersToString(request.getHeaders()));
			logPulito.append("\n\tRequest Body: " + new String(body, StandardCharsets.UTF_8));
			logPulito.append("\n========== REQUEST END ==========\n");
			log.info(logPulito.toString());
        }

        private void logResponse(ClientHttpResponse response, String requestId) throws IOException {
        	StringBuilder logPulito = new StringBuilder("\n========== RESPONSE BEGIN "+ requestId +" ==========");
        	logPulito.append("\n\tHTTP Status Code: " + response.getRawStatusCode());
        	logPulito.append("\n\tStatus Text: " + response.getStatusText());
        	logPulito.append("\n\tHTTP Headers: " + headersToString(response.getHeaders()));
        	logPulito.append("\n\tResponse Body: " + bodyToString(response.getBody()));
        	logPulito.append("========== RESPONSE END ==========\n");
        	log.info(logPulito.toString());
        }
	...
	public JwtTokenHelperInterface getJwtTokenHelper() {
		return jwtTokenHelper;
	}

	public void setJwtTokenHelper(JwtTokenHelperInterface jwtTokenHelper) {
